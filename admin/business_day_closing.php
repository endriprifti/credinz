<?php
/*
  Credins Bank E-Commerce Solutions
  http://www.bankacredins.com

  Copyright (c) 2013 Endri Prifti

  Credinz Payment Module for osCommerce 2.3
*/
 	...
 	if(tep_not_null($action)) {
		switch($action) {
			case 'closebday':
				...
				tep_db_query("INSERT INTO `credinz_closing_business_day`(`result`, `result_code`, `count_reversal`, `count_transaction`, `amount_reversal`, `amount_transaction`, `response`) VALUES ('" . $response_array[1] . "','" . $response_array[3] . "'," . $response_array[7] . "," . $response_array[9] . "," . ($response_array[15]/100) . "," . ($response_array[17]/100) . ",'" . $resp . "')");
...
<?php
	$orders_query_raw = "select * from credinz_closing_business_day order by close_date desc";
