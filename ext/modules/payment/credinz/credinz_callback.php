...
*/
  ...
  $merchant	= new Merchant(	MODULE_PAYMENT_CREDINZ_URL_SERVER_HANDLER,
	...
	if($resp[1] !== 'OK') {
		...
		tep_db_perform('credinz_transaction', $sql_data_array, 'update', 'trans_id="' . $HTTP_POST_VARS['trans_id'] . '"');

		tep_db_query("update " . TABLE_ORDERS . " set orders_status = '" . MODULE_PAYMENT_CREDINZ_FAILED_STATUS_ID . "', last_modified = now() where orders_id = '" . (int)$HTTP_POST_VARS['order_id'] . "'");

		$sql_data_array = array('orders_id'			=> $HTTP_POST_VARS['order_id'],
								'orders_status_id'	=> MODULE_PAYMENT_CREDINZ_FAILED_STATUS_ID,
								'date_added'		=> 'now()',
								'customer_notified'	=> '0',
								'comments'			=> 'Credins Bank: Transaction NOT Valid');
		...
		tep_redirect(
					tep_href_link(	
									...
									'payment_error=credinz&error=' . urlencode(MODULE_PAYMENT_CREDINZ_ERROR_TRANS_NOT_SUCCEEDED), 
									'SSL'
					)
		);
		exit();
	...
	if (isset($HTTP_POST_VARS['hash'])	&& 
		...
		($HTTP_POST_VARS['hash'] 		== md5(	MODULE_PAYMENT_CREDINZ_PASSPHRASE	.
        ...
        if (tep_db_num_rows($order_query) > 0) {
          ...
          if ($order['orders_status'] == MODULE_PAYMENT_CREDINZ_PREPARE_ORDER_STATUS_ID) {
            tep_db_query("update " . TABLE_ORDERS . " set orders_status = '" . MODULE_PAYMENT_CREDINZ_WAITING_STATUS_ID . "', last_modified = now() where orders_id = '" . (int)$HTTP_POST_VARS['order_id'] . "'");

            $sql_data_array = array('orders_id'			=> $HTTP_POST_VARS['order_id'],
                                    'orders_status_id'	=> MODULE_PAYMENT_CREDINZ_PREPARE_ORDER_STATUS_ID,
                                    'date_added'		=> 'now()',
                                    'customer_notified'	=> '0',
                                    'comments'			=> '');
            ...
            $sql_data_array = array('orders_id'			=> $HTTP_POST_VARS['order_id'],
                                    'orders_status_id'	=> MODULE_PAYMENT_CREDINZ_WAITING_STATUS_ID,
                                    'date_added'		=> 'now()',
                                    'customer_notified'	=> '0',
                                    'comments'			=> 'Credins Bank: Transaction Verified');
		...
		tep_redirect(
					tep_href_link(	
									FILENAME_CHECKOUT_PAYMENT, 
									'payment_error=credinz&error=' . urlencode(MODULE_PAYMENT_CREDINZ_HACK_ATTEMPTED), 
									'SSL'
					)
		);
		...
		tep_redirect(
					tep_href_link(	
									FILENAME_CHECKOUT_PAYMENT, 
									'payment_error=credinz&error=' . urlencode(MODULE_PAYMENT_CREDINZ_NO_TRANS_ID),
									'SSL'
					)
		);
	}
