...
*/
  ...
  class Credinz {
    ...
    function Credinz() {
      ...
      $this->signature = 'credinz|sms|1.0';
      $this->api_version = '1.0';

      $this->code        = 'credinz';
      $this->title       = MODULE_PAYMENT_CREDINZ_TEXT_TITLE;
      $this->description = MODULE_PAYMENT_CREDINZ_TEXT_DESCRIPTION;
      $this->sort_order  = MODULE_PAYMENT_CREDINZ_SORT_ORDER;
      $this->enabled     = ((MODULE_PAYMENT_CREDINZ_STATUS == 'True') ? true : false);

      if ((int)MODULE_PAYMENT_CREDINZ_PREPARE_ORDER_STATUS_ID > 0) {
        $this->order_status = MODULE_PAYMENT_CREDINZ_PREPARE_ORDER_STATUS_ID;
      }

      if (is_object($order)) $this->update_status();

      $this->form_action_url = MODULE_PAYMENT_CREDINZ_URL_CLIENT_HANDLER;
    }

    function update_status() {
      global $order;

      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_CREDINZ_ZONE > 0) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_CREDINZ_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      global $cart_Credinz_ID;

      if (tep_session_is_registered('cart_Credinz_ID')) {
        $order_id = substr($cart_Credinz_ID, strpos($cart_Credinz_ID, '-')+1);
        ...
        if (tep_db_num_rows($check_query) < 1) {
          ...
          tep_session_unregister('cart_Credinz_ID');
        }
      }

      return array(	'id'		=> $this->code, 
	  				'module'	=> $this->title, 
					'fields'	=> array(array('title' => '', 'field' => MODULE_PAYMENT_CREDINZ_TEXT_PUBLIC_HTML)));
    }
    ...
    function confirmation() {
      global $cartID, $cart_Credinz_ID, $customer_id, $languages_id, $order, $order_total_modules;
      ...
      if (tep_session_is_registered('cartID')) {
        ...
        if (tep_session_is_registered('cart_Credinz_ID')) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($cart_Credinz_ID, 0, strlen($cartID))) ) {
            $check_query = tep_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
        ...
        if ($insert_order == true) {
          ...
          $cart_Credinz_ID = $cartID . '-' . $insert_id;
          tep_session_register('cart_Credinz_ID');
        }
      }
    ...
    function process_button() {
      global $order, $currencies, $customer_id, $languages_id, $cart_Credinz_ID, $trans_id, $ip;
...
return	tep_draw_hidden_field('hash',			md5(MODULE_PAYMENT_CREDINZ_PASSPHRASE	.	//Passphrase
													tep_session_id()					.	//Session ID
													...
													$ip	)								)	//IP
																	.
		tep_draw_hidden_field('session_id',		tep_session_id())	.	//Session		ID
		tep_draw_hidden_field('trans_id',		$trans_id)			.	//Transaction	ID
		tep_draw_hidden_field('customer_id',	$customer_id)		.	//Customer		ID
		tep_draw_hidden_field('order_id',		$order_id)			.	//Order			ID

		tep_draw_hidden_field('amount',			$amount)			.	//Amount
		tep_draw_hidden_field('ip',				$ip)				.	//IP
    ...
    function before_process() {
      global $customer_id, $language, $order, $order_totals, $sendto, $billto, $languages_id, $payment, $currencies, 
	  		 $cart, $cart_Credinz_ID, $is_transaction_verified;
      global $$payment;
      ...
      $sql_data_array = array('orders_id'			=> $order_id,
                              'orders_status_id'	=> (($is_transaction_verified===true) ? 
								  			(int)MODULE_PAYMENT_CREDINZ_WAITING_STATUS_ID : (int)MODULE_PAYMENT_CREDINZ_FAILED_STATUS_ID),
                              'date_added'			=> 'now()',
      ...
      if($is_transaction_verified===true) tep_redirect(tep_href_link(FILENAME_CHECKOUT_SUCCESS, '', 'SSL'));
					...
					tep_href_link(	
									...
									'payment_error=credinz&error=' . urlencode(MODULE_PAYMENT_CREDINZ_NO_PAYMENT), 
									'SSL'
					)
		);
    }
    ...
    function get_error() {
      ...
      $error = array('title' => MODULE_PAYMENT_CREDINZ_ERROR_HEADING,
                     'error' => ((isset($HTTP_GET_VARS['error']) && tep_not_null($HTTP_GET_VARS['error'])) ? stripslashes(urldecode($HTTP_GET_VARS['error'])) : MODULE_PAYMENT_CREDINZ_ERROR_MESSAGE));
    ...
    function install() {
		$status_id				= $this->set_credinz_order_statues("[Credins] Preparing Transaction");
		$status_delivered_id	= $this->set_credinz_order_statues("[Credins] Delivered");
		$status_failed_id		= $this->set_credinz_order_statues("[Credins] Failed Transaction");
		$status_waiting_id		= $this->set_credinz_order_statues("[Credins] Paid, waiting delivery");

      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Credins Bank Module', '" . $this->keys(0) . "', 'True', 'Do you want to accept Credins Bank payments?', '6', '3', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Keystore path', '" . $this->keys(1) . "', '" .  DIR_FS_CATALOG . "ext/modules/payment/credinz/II000001_imakstore_test.pem" . "', 'The keystore file is needed to connect the osCommerce from the merchant server with the payment gateway.<br />You need to give the full path.<br />Do put a single slash.', '6', '4', now())");
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Currency ISO Code', '" . $this->keys(5) . "', '008', 'The currency of the merchant\'s account.<br />The ISO Code of Albanian Lek is 008.', '6', '4', now())");
     ...
     tep_db_query(
		  "CREATE TABLE IF NOT EXISTS `credinz_closing_business_day` (
	   ...
	   tep_db_query(
		  "CREATE TABLE IF NOT EXISTS `credinz_error` (
	   ...
	   tep_db_query(
		  "CREATE TABLE IF NOT EXISTS `credinz_transaction` (
		  `id`							int(10) 		NOT NULL AUTO_INCREMENT,
		  ...
		  `credinz_order_id`			int(11) 		NOT NULL,
		  `credinz_customer_id` 		int(11) 		NOT NULL,
		  `credinz_customer_name`		varchar(255)	NOT NULL,
		  `credinz_customer_surname`	varchar(255)	NOT NULL,
		  `credinz_timestamp` 			timestamp 		NOT NULL
	...
	function set_credinz_order_statues($order_status_name) {
    ...
    function keys($index = -1) {
		$array_of_keys = array(
			'MODULE_PAYMENT_CREDINZ_STATUS',			//0

			'MODULE_PAYMENT_CREDINZ_KEYSTORE',			//1
			'MODULE_PAYMENT_CREDINZ_PASSPHRASE',		//2
			'MODULE_PAYMENT_CREDINZ_URL_SERVER_HANDLER',//3
			'MODULE_PAYMENT_CREDINZ_URL_CLIENT_HANDLER',//4
			'MODULE_PAYMENT_CREDINZ_CURRENCY_ISO_CODE',	//5

			'MODULE_PAYMENT_CREDINZ_SORT_ORDER',		//6
			'MODULE_PAYMENT_CREDINZ_ZONE',				//7
			'MODULE_PAYMENT_CREDINZ_ORDER_STATUS_ID',	//8

			'MODULE_PAYMENT_CREDINZ_PREPARE_ORDER_STATUS_ID',	//9
			'MODULE_PAYMENT_CREDINZ_DELIVERED_STATUS_ID',		//10
			'MODULE_PAYMENT_CREDINZ_FAILED_STATUS_ID',			//11
			'MODULE_PAYMENT_CREDINZ_WAITING_STATUS_ID'			//12
		);
	...
	private function get_transaction_id($amount, 
										...
										$credinz_order_id, 
										$credinz_customer_id, 
										$credinz_customer_name,
										$credinz_customer_surname) {
		...
		$merchant	= new Merchant(	MODULE_PAYMENT_CREDINZ_URL_SERVER_HANDLER,
									...
									1);
		$resp		= $merchant->startSMSTrans($amount * 100, MODULE_PAYMENT_CREDINZ_CURRENCY_ISO_CODE, $ip, $description, $language);

		if(substr($resp,0,14) == "TRANSACTION_ID") {
			$trans_id	= substr($resp,16,28);

			tep_db_query("INSERT INTO credinz_transaction 
						  SET trans_id					='" . $trans_id . "', 
						      amount					='" . $amount . "', 
							  currency					='" . MODULE_PAYMENT_CREDINZ_CURRENCY_ISO_CODE . "', 
							  client_ip_addr			='" . $ip . "', 
							  description				='" . $description . "', 
							  credinz_order_id			='" . $credinz_order_id . "', 
							  credinz_customer_id		='" . $credinz_customer_id . "', 
							  credinz_customer_name		='" . $credinz_customer_name . "', 
							  credinz_customer_surname	='" . $credinz_customer_surname . "', 
							  language					='" . $language . "', 
							  dms_ok					='---', 
							  result					='???', 
							  result_code				='???', 
		...
		else tep_db_query("INSERT INTO credinz_error VALUES (NULL, now(), 'startsmstrans', '" . htmlentities($resp, ENT_QUOTES) . "')");
